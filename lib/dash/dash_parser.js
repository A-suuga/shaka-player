...
goog.require('shaka.util.CozyUtils');
...
/**
 * @typedef {
 *   function(!Promise.<!Array.<string>>, ?number, ?number):!Promise.<!ArrayBuffer>
 * }
 */
 ...
 * @property {!Promise.Array.<string>} baseUris
...
shaka.dash.DashParser.prototype.processManifest_ =
  ...
  const baseUrisP =
  /** @type {!Promise.<!Array.<string>>} */ (Promise.resolve(baseUris));
  ...
  let periodsAndDuration = this.parsePeriods_(context, baseUrisP, mpd);
  ...
  return this.parseUtcTiming_(
      baseUrisP, timingElements, isLive).then(function(offset) {
    // Detect calls to stop().
    if (!this.playerInterface_) {
      return;
    }
...
shaka.dash.DashParser.prototype.parseAdaptationSet_ = function(context, elem) {
  ...
  context.adaptationSet = this.createFrame_(elem, context.period, new Promise(
    () => null
  ));
...
shaka.dash.DashParser.prototype.parseRepresentation_ = function(
  ...
  context.representation = this.createFrame_(node, context.adaptationSet,
    new Promise(() => null));
...
shaka.dash.DashParser.prototype.createFrame_ = function(
  ...
  // let baseUrisPromise;
  // if(uris.length == 0) {
  //   baseUrisPromise = Promise.resolve(
  //     ManifestParserUtils.resolveUris(baseUris, uris));
  // } else {
  //   // baseUris = ManifestParserUtils.cozyUris(baseUris, uris);
  //   let baseUrisOperation = shaka.util.CozyUtils.resolveCozyUris(
  //     baseUris, uris, this.config_.retryParameters,
  //     this.playerInterface_.networkingEngine);
  //
  //   this.operationManager_.manage(baseUrisOperation);
  //   baseUrisPromise = baseUrisOperation.promise.then((finalbaseUris) => {
  //     return finalbaseUris;
  //   });
  // }
  let baseUrisPromise;
  if (uris.length == 0) {
    baseUrisPromise = baseUris;
  } else {
    const baseUrisOperation = shaka.util.CozyUtils.resolveCozyUris(
      baseUris, uris, this.config_.retryParameters,
      this.playerInterface_.networkingEngine,
      this.playerInterface_.cozyInfo);

    this.operationManager_.manage(baseUrisOperation);
    baseUrisPromise = baseUrisOperation.promise;
  }

  return {
    baseUris: baseUrisPromise,
...
shaka.dash.DashParser.prototype.requestForTiming_ =
  ...
  let requestUris = shaka.util.ManifestParserUtils.resolveUrisPromise(
    baseUris, [uri]);

  // let requestUris = shaka.util.CozyUtils.resolveCozyUris(
  //   baseUris, [uri], this.config_.retryParameters,
  //   this.playerInterface_.networkingEngine);

  // this.operationManager_.manage(requestUris);
  // return requestUris.promise.then((finalRequestUris) => {
  return requestUris.then((finalRequestUris) => {
    let request = shaka.net.NetworkingEngine.makeRequest(
        finalRequestUris, this.config_.retryParameters);
 ...
 *
 * @param {!Promise.<!Array.<string>>} uris
...
shaka.dash.DashParser.prototype.requestInitSegment_ = function(
  ...
  return uris.then((finalUris) => {
    let request = shaka.net.NetworkingEngine.makeRequest(
        finalUris, this.config_.retryParameters);
    if (startByte != null) {
