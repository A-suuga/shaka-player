 ...
 * @param {shaka.util.CozyUtils.CozyInfo} cozyInfo
 * @return {!shaka.util.AbortableOperation.<!Array.<string>>}
 */
shaka.util.CozyUtils.resolveCozyUris =
  function(baseUris, relativeUris, retryParameters, networkingEngine, cozyInfo) {
  // if (relativeUris.length == 0) {
  //   return shaka.util.AbortableOperation.completed(baseUris);
  // }

  shaka.log.warning('Domain: ', cozyInfo.domain);
  shaka.log.warning('Token: ', cozyInfo.token);
  ...
  return CozyUtils.defineFileIndex(
    ['name'], cozyInfo, retryParameters, networkingEngine)
  .chain((indexRef) => {
    const opts = {'selector': {name: fileName} };
    return CozyUtils.query(
      indexRef, opts, cozyInfo, retryParameters, networkingEngine);
  })
  .chain((docs) => {
    ...
    return CozyUtils.getDownloadLinkById(
      doc['_id'], cozyInfo, retryParameters, networkingEngine);
  });
 ...
 */
 shaka.util.CozyUtils.query =
    function(indexRef, opts, cozyInfo, retryParameters, networkingEngine) {
   const CozyUtils = shaka.util.CozyUtils;
   ...
   const path = '//' + cozyInfo.domain + '/data/' + indexRef.doctype + '/_find';

   const request = CozyUtils.cozyRequest(
     cozyInfo.token, retryParameters, 'POST', path, allOptsStringified);
...
shaka.util.CozyUtils.defineFileIndex =
  function(fields, cozyInfo, retryParameters, networkingEngine) {

  const path = '//' + cozyInfo.domain + '/data/' + 'io.cozy.files' + '/_index';
  ...
  const request = shaka.util.CozyUtils.cozyRequest(
    cozyInfo.token, retryParameters, 'POST', path, wrappedJson);
  ...
  .chain((indexRes) => {
    ...
    const path = '//' + cozyInfo.domain + '/data/' + 'io.cozy.files' + '/_find';
...
shaka.util.CozyUtils.getDownloadLinkById = function(
    id, cozyInfo, retryParameters, networkingEngine) {
  const CozyUtils = shaka.util.CozyUtils;

  const path = '//' + cozyInfo.domain + '/files/downloads?Id=' + id;

  const request = CozyUtils.cozyRequest(
    cozyInfo.token, retryParameters, 'POST', path, null);
  const networkOperation = CozyUtils.cozyOperation(
    networkingEngine, request);
  ...
  return networkOperation.chain((response) => {
    ...
    const fullFileUri = '//' + cozyInfo.domain + fileUri;
